...
struct LogCore {
    ...
    frozen_files: RefCell<Vec<File>>,
    first_log_file_id: Cell<u64>,

    queue: RefCell<Vec<Writer>>,

    lock: RwLock<PhantomData<()>>,
}
...
impl Log {
    ...
    pub async fn append(&self, entries: Vec<Entry>) -> Result<WriteHandle> {
        ...
        let is_leader = {
            let _ = self.core.lock.write();
            let mut queue_mut = self.core.queue.borrow_mut();
            let is_leader = queue_mut.is_empty();
            queue_mut.push(writer);
            is_leader
        };
        ...
        if is_leader {
            ...
            let writers = {
                let _ = self.core.lock.write();
                let mut queue_mut = self.core.queue.borrow_mut();
                let writers = std::mem::take(&mut (*queue_mut));
                writers
            };
            ...
            for writer in writers {
                ...
                let file_id = self.core.first_log_file_id.get()
                    + self.core.frozen_files.borrow().len() as u64;
                ...
                if offset + len >= self.log_file_capacity {
                    ...
                    *file = new_active_file(&self.path, new_active_file_id).await?;
                    ...
                    self.core.frozen_files.borrow_mut().push(frozen_file);
                }
...
mod tests {
    ...
    async fn test_pipe_log_recovery() {
        ...
        let log = LogV1::open(options.clone()).await.unwrap();
        ...
        // Recover pipe log.
        drop(log);
        let log = LogV1::open(options).await.unwrap();
